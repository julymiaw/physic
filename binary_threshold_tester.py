...
def process_frame_for_threshold_test(frame, manual_threshold=128):
    """
    按照轮廓检测流程处理帧，先裁剪有效区域，再用手动阈值进行二值化测试
    """
    try:
        # 1. 提取红色通道
        red_channel = frame[:, :, 2]
        ...
        # 4. 使用手动阈值进行二值化（注意这里是THRESH_BINARY_INV，与原始代码一致）
...
class ThresholdAfterCropTester:
    ...
    def setup_ui(self):
        ...
        ttk.Label(
            right_image_frame,
            text="Binary Image (THRESH_BINARY_INV)",
            font=("Arial", 12),
        ).pack()
    ...
    def update_binary_image_only(self):
        ...
        # 使用当前阈值进行二值化
        _, binary_image = cv.threshold(
            self.cropped_red_channel, self.current_threshold, 255, cv.THRESH_BINARY_INV
        )
    ...
    def update_display(self):
        ...
        (
            ...
            binary_image,
            bbox_area,
        ) = process_frame_for_threshold_test(frame, self.current_threshold)
        ...
        if cropped_frame is not None:
            ...
            status_text = f"Success - Cropped region: {w}x{h}, Area: {bbox_area}, Manual: {self.current_threshold}"
        else:
            status_text = f"Processing failed - Manual: {self.current_threshold}"
